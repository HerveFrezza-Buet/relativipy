...
universe = rel.scene.Universe(screen_size = (3, 16), width=800, height=450)
...
ship = ship * np.array([1, -1]) + np.array([0, 7])
universe += rel.objects.Prism((0, - travel_speed), universe.C, (None, None), ship, (.2, .2, .2))
...
chrono_herve_B = rel.objects.Chronometer(None, universe.C, start_evt, chrono_period, end_time - start_evt[2] - epsilon, (.5, .2, .2))
start_evt[2] = end_time
